...
def run_model_analysis(_6mer,motifs,Base_model_name,n_models,device='cuda'):
    ...
    for model_name in model_names:
        ...
        for condition in cond_array:
            ...
            condition_label = condition[0].item()  
...
if __name__ == '__main__':
    run_model_analysis(_6mer=True,motifs=["TTCCCT", "CTACCT", "TGCAGT"],Base_model_name="BigFilterMaxPoolCond",n_models=2,device='cuda')
    #run_model_analysis(_6mer=False,n_models=10,device='cuda')
# Example usage
# run_model_analysis(_6mer=True, device='cuda', create_synth_test_data=..., evaluate_model=..., generate_nuc_seqs=..., ...)
...
#     model_names = [f'BigFilterMaxPoolCond_6merMotifs_{i}' for i in range(5)]
#     Base_model_name = 'BigFilterMaxPoolCond_6merMotifs'
# else:
#     motifs = ["TTCCCTCG", "CTACCTCC", "TGCAGTGC"]
#     model_names = [f'BigFilterMaxPoolCond_{i}' for i in range(5)]
#     Base_model_name = 'BigFilterMaxPoolCond'
# # Define conditions as tensors for the conditional model
# test_conditions = 100
# cond_array = [torch.zeros(test_conditions, device=device),
#               torch.ones(test_conditions, device=device),

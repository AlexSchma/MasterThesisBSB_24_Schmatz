...
def train_and_save( train_loader, val_loader,epochs=150,n_models=5,lr=0.001,L2=1e-5,_6mer=False,dropout_rate=0.2):
    ...
    for model_idx in range(n_models):
        ...
        if _6mer:
            ...
            torch.save(model, f'models/BigFilterMaxPoolCondEmbedding_6merMotifs_{model_idx}.pth')
            print(f"Model {model_idx} saved to BigFilterMaxPoolCondEmbedding_6merMotifs_{model_idx}.pth")
        else:
            torch.save(model, f'models/BigFilterMaxPoolCondEmbedding_{model_idx}.pth')
            print(f"Model {model_idx} saved to BigFilterMaxPoolCondEmbedding_{model_idx}.pth")

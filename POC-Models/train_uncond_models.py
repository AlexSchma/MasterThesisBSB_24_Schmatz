...
#creates syntehtic data and returns pytorch dataloaders

def train_and_save( train_loader, val_loader,epochs=70,n_models=5,_6mer=False):
    ...
    for model_idx in range(n_models):
        model = ModifiedSimpleCNN().to(device)
        ...
        if _6mer:
            ...
            torch.save(model, f'models/BigFilterMaxPool_6merMotifs_{model_idx}.pth')
            print(f"Model {model_idx} saved to BigFilterMaxPool_6merMotifs_{model_idx}.pth")
        else:
            torch.save(model, f'models/BigFilterMaxPool_{model_idx}.pth')
            print(f"Model {model_idx} saved to BigFilterMaxPool_{model_idx}.pth")
...
if __name__ == "__main__":
    ...
    train_loader, val_loader, test_loader = create_synth_data_uncond(motifs=motifs)
    train_and_save(train_loader, val_loader,epochs=70,n_models=5,_6mer=_6mer)

...
class ProCNN_multi_class(nn.Module):
    """
    A two layer CNN that uses max pooling to extract information from
    the sequence input, a one layer fully-connected network to extract TF information
    then trains a fully connected neural network on the to make predictions.
    ...
    def __init__(self, config, output_classes):
        super(ProCNN_multi_class, self).__init__()
...
def get_funprose_multi_class(
    ...
    conv_activation="relu",
    fc_activation="elu",
    conv_dropout=0.3981796388676127*0.8,
    tf_dropout=0.18859739941162465*0.8,
    fc_dropout=0.016570328292903613*0.8,
    n_treatments=3,
):
    config = {
        "input_channels": input_channels,
    ...
    model = ProCNN_multi_class(config, 3)
    #
    # X = torch.rand(2, 4, 4000)
    # Z = torch.rand(2, 3)

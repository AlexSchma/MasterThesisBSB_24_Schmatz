...
def sample_aug_combos(batch_size, hard_aug = True, max_augs_per_seq = 3 , max_num_aug = 6):
        ...
        aug_combos = [ list(sorted(np.random.choice(max_num_aug, sample, replace=False))) for sample in batch_num_aug ]
        return aug_combos

def apply_augment(x,augment_list,insert_max=30):
        """Apply augmentations to each sequence in batch, x."""
        # number of augmentations per sequence
        aug_combos = sample_aug_combos(x.shape[0])
        ...
        for aug_indices, seq in zip(aug_combos, x):
            ...
            if insert_status:
                if insert_max:
                    seq = pad_end(seq,insert_max)
            x_new.append(seq)
        return torch.cat(x_new)

def pad_end(x,insert_max=30):

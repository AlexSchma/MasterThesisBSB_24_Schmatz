...
def train(
    ...
    class EarlyStopper:
        def __init__(self, patience=1, min_delta=0):
            ...
            self.counter = 0
            self.max_validation_mcc = float("-inf")  # Track the maximum MCC

        def early_stop(self, validation_mcc):
            if validation_mcc > self.max_validation_mcc:
                self.max_validation_mcc = validation_mcc
                self.counter = 0
            elif validation_mcc < (self.max_validation_mcc - self.min_delta):
                self.counter += 1
    ...
    for epoch in range(num_epochs):
        ...
        if val_MCC > early_stopper.max_validation_mcc:
            torch.save(model.state_dict(), f"{path}/{name}_model_best_model.pth")
    
        if early_stopper.early_stop(val_MCC):
...
train(
    ...
    "up_down_multi_class_inc_dropout_MCC_early_stopp",
    1000,
    512,

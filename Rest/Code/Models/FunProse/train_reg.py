...
from Code.Models.CNNs.funprose_adapted_reg import get_funprose
...
with open("Data/Processed/XY_formats/One_hot_encoded_up_down_reg.pkl", "rb") as f:
...
def apply_thresholds(y_pred):
    # Apply thresholds to continuous outputs to classify as -1, 0, or 1
    return torch.sign(y_pred) * (torch.abs(y_pred) > 0.33).float()

def categorize_outputs(outputs):
    outputs = outputs.squeeze()
    # Example categorization based on assumed thresholds
    categorized_outputs = torch.zeros(outputs.size(0), 3)
    categorized_outputs[outputs <= -0.33, 0] = 1  # Class -1
    categorized_outputs[(outputs > -0.33) & (outputs < 0.33), 1] = 1  # Class 0
    categorized_outputs[outputs >= 0.33, 2] = 1  # Class 1
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        for batch in pbar:
            if only_index:
                ...
                batch_X = torch.tensor(batch["X"], dtype=torch.float).to(device)  # Batch of X sequence
            batch_Z = torch.tensor(batch["Z"], dtype=torch.float).to(device)  # Batch of X condition
            batch_Y = torch.tensor(batch["Y"], dtype=torch.float).to(device)  # Batch of label
            optimizer.zero_grad()
            ...
            predicted = apply_thresholds(outputs).squeeze()
            MCCs.append(matthews_corrcoef(batch_Y.cpu().numpy(), predicted.cpu().detach().numpy()))
           
            accuracies.append((predicted == batch_Y).sum().item() / batch_Y.size(0))
            losses.append(loss.item())
            classes = [-1, 0, 1]
            y_true_bin = label_binarize(batch_Y.cpu().numpy(), classes=classes)
            #y_pred_prob = torch.softmax(outputs, dim=1).cpu().detach().numpy()  # Softmax needed to convert to probabilities

            # Calculate AUC for each class and average (macro averaging)
            categorized_probs = categorize_outputs(outputs.cpu().detach())
            auc_scores = []
            for i in range(len(classes)):
                auc_score = roc_auc_score(y_true_bin[:, i], categorized_probs[:, i])
                auc_scores.append(auc_score)
            aucROC.append(np.mean(auc_scores))
        ...
        train_MCC = matthews_corrcoef(
            batch_Y.detach().cpu().numpy(), predicted.detach().cpu().detach().numpy()
        )
        # Validation passes
        with torch.no_grad():
            model.eval()
            ...
            for batch in pbar:
                if only_index:
                    ...
                    batch_X = torch.tensor(batch["X"], dtype=torch.float).to(
                        device
                    )  # Batch of X sequence
                batch_Z = torch.tensor(batch["Z"], dtype=torch.float).to(device)
                batch_Y = torch.tensor(batch["Y"], dtype=torch.float).to(device)
                outputs = model(batch_X, batch_Z)
                loss = criterion(outputs, batch_Y)
                predicted = apply_thresholds(outputs).squeeze()
                MCCs.append(matthews_corrcoef(batch_Y.cpu().numpy(), predicted.cpu().detach().numpy()))
    ...
    with torch.no_grad():  # Check performance test set of best model
        ...
        for batch in tqdm(data_loader_test):
            if only_index:
                batch_X = (
                    torch.tensor(data_npy[batch["X"]], dtype=float)
                    .to(device)
                    .transpose(1, 2)
                )  # Batch of X sequence (batch, channel, sequence)
            else:
            ...
            batch_Z = torch.tensor(batch["Z"], dtype=torch.float).to(
                device
        ...
        accuracy = np.mean(all_predictions.detach().cpu().detach().numpy() == Y_test)
        # Calculate AUC-ROC
        classes = [-1, 0, 1]
        y_true_bin = label_binarize(Y_test, classes=classes)
        #y_pred_prob = torch.softmax(all_probas, dim=1).cpu().detach().numpy()  # Convert logits to probabilities

        auc_scores = []
        categorized_probs = categorize_outputs(all_probas.cpu().detach())
        for i in range(len(classes)):
            auc_score = roc_auc_score(y_true_bin[:, i], categorized_probs[:, i])
            auc_scores.append(auc_score)
        ...
        skplt.metrics.plot_roc(Y_test, categorized_probs, title="ROC Curve")
        plt.savefig(f"{path}/{name}_roc_curve.png")

        skplt.metrics.plot_precision_recall(Y_test, categorized_probs)
        plt.savefig(f"{path}/{name}_precision_recall_curve.png")
        # Calculate MCC
        mcc = matthews_corrcoef(Y_test, all_predictions.cpu().detach().numpy())
        print(f"Test MCC : {mcc:.4f}")

        # Confusion Matrix
        skplt.metrics.plot_confusion_matrix(Y_test, all_predictions.cpu().detach().numpy(), normalize=False)
        plt.savefig(f"{path}/{name}_confusion_matrix.png")
        print(f"Test MCC : {mcc:.4f}")
        print(f"Test AUC-ROC : {average_auc:.4f}")
        ...
        if num_epochs == 0:
            return accuracy, mcc, average_auc

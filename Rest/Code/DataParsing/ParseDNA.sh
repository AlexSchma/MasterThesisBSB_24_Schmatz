...
awk -v upstream_bp="$UPSTREAM_BP" -v downstream_bp="$DOWNSTREAM_BP" 'BEGIN{FS = OFS ="\t"} $3=="gene" {
    if($7 == "+") {
        ...
        downstream_end = $4 + downstream_bp # It could be a better idea to handle this such that the downstream end is the end of the gene
    } else if($7 == "-") {
        upstream_start = ($5 > downstream_bp) ? $5 - downstream_bp -1 : 0
        downstream_end = $5 + upstream_bp # It could be a better idea to handle this such that the downstream end is the end of the gene
    } 
    split($9, id_array, /[=;]/)

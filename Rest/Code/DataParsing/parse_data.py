...
def generate_treatment_representations(directory, TFs=None):
    """
...
def construct_dataset(
    ...
    treatment_input_url,
    treatments=["B", "C", "D", "G", "H"],
    outcome_url="Data/RAW/DEGsUU/DEGs_treatments_vs_mock.txt",
    ...
    if raw_counts:
        ...
        outcome_df = pd.read_csv(outcome_url, sep="\t", index_col=0)
    mapping = {
        "ABA_vs_Mock": "G",
        "JA_vs_Mock": "B",
        "SA_vs_Mock": "C",
        "ABA_and_JA_vs_Mock": "H",
        "SA_and_JA_vs_Mock": "D",
    }
    outcome_df = outcome_df.rename(columns=mapping)
    for i in tqdm(promoter_rep_urls):
        ...
        if gene_name not in outcome_df.index:
            continue
    ...
    missing_genes = set(outcome_df.index) - set(gene_names)
    print("Gene names in outcome_df but not in promoter_rep_dir:")
    print(len(missing_genes))
    # Save the dataset as .csv
    dataset_df = pd.DataFrame(dataset, columns=["X", "Y", "Z"])
    dataset_df.to_csv("Data/Processed/dataset.csv", index=False)
    print("Dataset constructed and saved as Data/Processed/dataset.csv")
...
parser.add_argument(
    ...
    type=str,
    default="Data/Processed/One_hot",
...
parser.add_argument(
    ...
    type=str,
    default="Data/Processed/TreatmentInput",
    help="Output directory for the processed DNA sequences",
)
...
if args.oneHot:
    ...
    if args.introns:
        ...
        print("Number of introns above 2000: ", len([x for x in length_introns if x > 2000]))
    #introns_dict = load_and_concatenate_introns(url_introns)
...
if args.introns:
    ...
    for gene in tqdm(one_hot_promoters_tss.keys()):
        if gene not in one_hot_introns:
            one_hot_introns[gene] = os.path.join(args.outputDirProcessedDNA, "introns", "null_intron.npy")
        tuple_list.append((one_hot_promoters_tss[gene], one_hot_promoters_tts[gene], one_hot_introns[gene]))
else:
    tuple_list = []
    for gene in tqdm(one_hot_promoters_tss.keys()):
        if args.upstreamTTS != 0 or args.downstreamTTS != 0:
            tuple_list.append((one_hot_promoters_tss[gene], one_hot_promoters_tts[gene]))
        else:
            tuple_list.append(one_hot_promoters_tss[gene])
# 4. Construct dataset
generate_treatment_representations(args.outputDirTreatmentInput, TFs=None)
